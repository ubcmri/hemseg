function imNav_Otsu(data,varargin)
...
switch event.Character
    ...
    case {'-'}
        ...
        blob_mask = grayconnected_cross(label_img,y,x,0);
        modified_mask = input_mask(:,:,curr_slice);
    ...
    case {'q'}
        output_mask = handles.input_mask;
        answer = inputdlg('Add entire label to current slice:','Index of added label',1,{num2str(1)});
        ...
        chosen_label = str2num(answer{1});

        modified_mask = input_mask(:,:,curr_slice);
        modified_mask(label_img==chosen_label) = 1;
        input_mask(:,:,curr_slice) = modified_mask;
    ...
    case {'w'}
        output_mask = handles.input_mask;
        answer = inputdlg('Delete entire label to current slice:','Index of deleted label',1,{num2str(1)});
        ...
        modified_mask = input_mask(:,:,curr_slice);
        modified_mask(label_img==chosen_label) = 0;
        input_mask(:,:,curr_slice) = modified_mask;
