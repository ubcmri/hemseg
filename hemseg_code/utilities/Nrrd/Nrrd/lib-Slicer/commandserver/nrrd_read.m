function img = nrrd_read(filename,varargin)
...
%   img = nrrdread(filename,isDataOnly) output only the pixel data
%
...
%
%  Modification by Andrew Yung (UBC MRI Research Centre): added the
%  isDataOnly input argument
%
...
ip.addOptional('isDataOnly',false);

ip.parse(filename,varargin{:});

isDataOnly = ip.Results.isDataOnly;
...
switch (meta.encoding)
    ...
    case {'gzip', 'gz'}
        if isfield(meta,'data_file')
            [nhdr_file_path,~,~,~] = fopen(fidIn);
            [data_path,~,~] = fileparts(nhdr_file_path);
            gz_file_path = [data_path filesep meta.data_file];
            gz_fid = fopen(gz_file_path);
            compressedData  = fread(gz_fid, inf, 'uchar=>uint8');
            try
                data = zlib_decompress(compressedData,datatype);
            catch noMemory
                error(['Not enough Java heap space or ' gz_file_path ' does not exist']);
                return;
            end
        else
            % if the nrrd header refers to a gzip datafile that doesn't
            % exist, assume that the nrrd file actually contains compressed
            % data that should be decompressed
            compressedData = fread(fidIn, inf, 'uchar=>uint8');
            try
                data = zlib_decompress(compressedData,datatype);
            catch noMemory
                error('Not enough Java heap space');
                return;
            end
