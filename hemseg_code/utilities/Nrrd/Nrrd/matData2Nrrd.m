...
ip.addParameter('sequence_times',[]);
ip.addParameter('sequence_time_unit','sec');
ip.addParameter('isMultivolume',false);
...
sequence_times = ip.Results.sequence_times;
sequence_time_unit = ip.Results.sequence_time_unit;
isMultivolume = ip.Results.isMultivolume;
...
dim = size(imdata);
if ~isempty(sequence_times) && length(dim)==4
    isSequence = true;
    assert(length(sequence_times) == dim(4));
else
    isSequence = false;
end
...
if isMultivolume
    output_im.metaData.MultiVolume_NumberOfFrames = dim(4);
end
...
if isSequence
    output_im.metaData.DataNodeClassName = 'vtkMRMLScalarVolumeNode';
    output_im.metaData.REPSPACEaxis_0_index_type = 'numeric';
    output_im.metaData.REPSPACEaxis_0_index_values = sequence_times;
    output_im.metaData.labels = '"time" "" "" ""';
    output_im.metaData.units = ['"' sequence_time_unit '" "" ""'];
    [~,filename,ext] = fileparts(dest_filename);
    dest_filename = [filename '.seq' ext];
